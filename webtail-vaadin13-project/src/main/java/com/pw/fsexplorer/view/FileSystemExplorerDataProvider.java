...
public class FileSystemExplorerDataProvider extends AbstractBackEndHierarchicalDataProvider<File, FilenameFilter> {
	...
	public FileSystemExplorerDataProvider(File root) {
		this.root = root;
	}
	...
	protected Stream<File> fetchChildrenFromBackEnd(HierarchicalQuery<File, FilenameFilter> query) {
		...
		Stream<File> filteredFiles = query.getFilter().map(filter -> Stream.of(parent.listFiles(filter)))
				.orElse(Stream.of(parent.listFiles())).skip(query.getOffset()).limit(query.getLimit());
	...
	private Stream<File> sortFileStream(Stream<File> fileStream, List<QuerySortOrder> sortOrders) {
		...
		List<Comparator<File>> comparators = sortOrders.stream().map(sortOrder -> {
		...
		Comparator<File> combinedComparators = comparators.stream().reduce(first, Comparator::thenComparing);
